...
<InternalPageMenu>
  ...
  <InternalPageMenuItem> Création de votre compte sous Gitlab </InternalPageMenuItem>
  ...
  <InternalPageMenuItem> Est-ce que cette intégration continue est vraiment utile ? </InternalPageMenuItem>
</InternalPageMenu>

# <InternalPageTitle> Création de votre compte Gitlab </InternalPageTitle>

Pour les étudiants Vinci, vous devez créer un compte Gitlab. Ce paragraphe est donc pour vous.
...
Gitlab propose des fonctionnalités intégrées pour mettre en place l'intégration continue de code. Voici les étapes à suivre pour configurer l'intégration continue dans Gitlab :
...
Les paramètres pour configurer la protection de la branche principale, la gestion des MR et des builds sont accessibles dans les paramètres du projet, ou dans les paramètres du groupe pour appliquer des règles à plusieurs projets. Pour les étudiants Vinci, ces paramètres ont été configurés par vos enseignants. 
...
- vous recevrez un email confirmant que vous avez été ajouté au projet ` e-vinci / cae-projects / 2025-cae-projects / cae-group-xy` avec le rôle de `Developer`.
- ce rôle de `Developer` vous permettra de créer des *merge requests* et de les fusionner. Vous pouvez également configurer le pipeline d'intégration continue pour exécuter les tests et vérifier la qualité du code. 
- par contre, vous ne pourrez pas configurer les règles de protection de la branche principale, ni les règles de *merge requests* au niveau du groupe. Ces règles ont été configurées par vos enseignants.

Les prochaines sections expliquent, étape par étape, comment configurer l'intégration continue dans Gitlab si vous aviez un rôle de `Maintainer` ou d'`Owner` au niveau d'un groupe Gitlab. Ces sections sont à titre informatif pour les étudiants Vinci.
...
Pour les étudiants Vinci, vous ne devez pas créer de Projet Gitlab. Ce paragraphe est donc pour compréhension uniquement du rôle d'un `Maintainer` ou d'un `Owner` qui doit créer un projet Gitlab.

Vous pouvez créer un nouveau projet `privé` sur Gitlab en suivant ces étapes :
...
3. Sélectionnez `Create blank project`.
...
Pour les étudiants Vinci, vous n'avez pas de projet à configurer. Ce paragraphe est donc pour compréhension uniquement du rôle d'un `Maintainer` ou d'un `Owner` au niveau d'un projet Gitlab.
...
Pour vérifier que tout est en ordre, vous pouvez suivre les étapes suivantes :
1. Accédez aux paramètres du projet dans Gitlab (`Settings`).
2. Cliquez sur `Repository` dans le menu de gauche.
...
1. Un développeur crée une nouvelle feature branche à partir de la branche principale.
2. Le développeur travaille sur sa feature et pousse ses modifications sur la branche distante.
3. Le développeur crée une *merge request* pour fusionner sa branche avec la branche principale.
4. Un autre membre de l'équipe approuve la *merge request*.

Nous allons commencer par ajouter le boilerplate de votre API Spring à votre projet Gitlab.

## Ajout du boilerplate de votre API Spring

Veuillez cloner votre projet Gitlab sur votre machine locale.

Vous devriez créer une nouvelle branche à partir de la branche principale pour ajouter le boilerplate de votre API Spring. Nous vous proposons de l'appeler `api-init`.
...
Après avoir fait ces modifications, vous devez pousser votre branche `api-init` sur votre repo distant (qui est votre projet Gitlab).

Comment créer une Merge Request ? [Voir la documentation](https://docs.gitlab.com/ee/user/project/merge_requests/creating_merge_requests.html)

Sur votre projet Gitlab, vous devriez voir un bouton `Create merge request` pour créer une *merge request*.

TBC : Vérifier les consignes qui suivent.
...
- **Reviewers** : Sélectionnez un membre de votre équipe pour approuver la *merge request*.
- **Merge options** : 
  - `Delete source branch when merge request is accepted.` : Cochez cette option pour supprimer la branche `boilerplate-init` après la fusion. On garde malgré tout l'historique complet des commits.
  - `Squash commits when merge request is accepted.` : On pourrait cocher cette option pour fusionner tous les commits de la branche `boilerplate-init` en un seul commit. Néanmoins, pour pouvoir suivre tous vos commits, nous vous demandons de ne pas cocher cette option.
- **Labels** : Ajoutez le label `boilerplate`.
- Cliquez sur `Create merge request`.


TBC : Vérifier les consignes qui suivent.
Veuillez demander à un autre membre de votre équipe de revoir et d'approuver votre *merge request*.

Comment approuver une Merge Request ? [Voir la documentation](https://docs.gitlab.com/ee/user/project/merge_requests/approvals/)

Une fois la *merge request* approuvée, vous ne pouvez toujours pas fusionner la branche `boilerplate-init` avec la branche principale.

Vous avez le message suivant : `Merge blocked: 1 check failed. The pipeline for this merge request failed.`
Cela signifie que le pipeline d'intégration continue a échoué. 

Et c'est normal, car nous n'avons pas encore configuré le pipeline d'intégration continue pour exécuter les tests et vérifier la qualité du code.
...
A la racine de votre Projet Gitlab (votre repository), dans la branche `api-init`, créez un fichier `.gitlab-ci.yml` avec le contenu suivant :
...
Maintenant que vous avez ajouté le fichier `.gitlab-ci.yml`, vous devriez voir un pipeline se déclencher automatiquement à chaque *push* sur la branche `api-init`.
...
Nous savons donc que la qualité du commit est bonne et que nous pouvons fusionner la branche `api-init` avec la branche principale.

## Fusion de la branche `api-init` avec la branche principale

Maintenant que le pipeline a été exécutée avec succès, vous pouvez fusionner la branche `api-init` avec la branche principale. Pour cela, vous devez approuver la *merge request*.

Une fois la *merge request* approuvée, vous pouvez cliquer sur le bouton `Merge` pour fusionner la branche `api-init` avec la branche principale.
...
Il reste à terminer la fusion de la branche `api-init` avec la branche principale sur votre machine locale.
Pour ce faire, vous devez vous placer sur la branche principale et récupérer les dernières modifications. Par exemple, si vous êtes sur la branche `api-init`, vous pouvez exécuter les commandes suivantes :
...
1. Un développeur crée une nouvelle feature branche à partir de la branche principale.
2. Le développeur travaille sur sa feature et pousse ses modifications sur la branche distante.
3. Le pipeline d'intégration continue est déclenché automatiquement. Si le pipeline échoue, le développeur doit corriger les erreurs avant de continuer.
4. Le développeur crée une *merge request* pour fusionner sa branche avec la branche principale.
5. Un autre membre de l'équipe approuve la *merge request*.
6. La *merge request* est fusionnée avec la branche principale.
...
Pour l'ajout du boilerplate de votre frontend, vous devez avoir fait tout ce qui est demandé dans la page [Qualité du Frontend](../quality/frontend).
...
Pour vous aider, nous avons créé un boilerplate pour le frontend qui reprend les tutoriels précédents et qui intègre la librairie MUI. Vous pouvez le trouver ici : [cae-project-boilerplate](https://github.com/e-vinci/cae-project-boilerplate).

Vous devriez créer une nouvelle branche à partir de la branche principale pour ajouter le boilerplate de votre frontend React dans un dossier nommé **`/frontend`**. Nous vous proposons d'appeler votre nouvelle branche `frontend-init`.
...
Après avoir poussé vos modifications, vous devriez voir un nouveau pipeline se déclencher automatiquement à chaque *push* sur la branche `frontend-init`.

Vous devriez voir deux jobs `api test` et `frontend test` qui sont exécutés en
parallèle.

Une fois le pipeline exécuté avec succès, nous pourrions fusionner la branche `frontend-init` avec la branche principale.
...
Maintenant que vous avez ajouté les artefacts dans votre pipeline, vous pouvez fusionner la branche `frontend-init` avec la branche principale.

Pour cela, vous devez créer une *merge request* pour fusionner la branche `frontend-init` avec la branche principale. Une fois la *merge request* approuvée, vous pouvez cliquer sur le bouton `Merge` pour fusionner la branche `frontend-init` avec la branche principale.

N'oubliez pas de mettre à jour votre branche principale sur votre machine locale après la fusion.

# <InternalPageTitle> Est-ce que cette intégration continue est vraiment utile ? </InternalPageTitle>
...
Veuillez donc créer une nouvelle branche au niveau du frontend que nous allons appeler `update-login-color`.
...
Après avoir poussé vos modifications, vous devriez voir un nouveau pipeline se déclencher automatiquement à chaque *push* sur la branche `update-login-color`.
