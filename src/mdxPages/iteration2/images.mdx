...
Si l'on souhaite permettre aux utilisateurs de télécharger des images, cette approche n'est pas adaptée.
...
Cette approche est principalement utile pour des assets qui sont statiques. Si l'on souhaite stocker des assets dynamiquement (ex: téléchargement d'images par les utilisateurs), il va falloir créer des opérations dédiées (cas suivant) dans l'API pour gérer ces assets.
...
Cette approche est particulièrement utile lorsque l'on souhaite stocker des assets dynamiquement (ex: téléchargement d'images par les utilisateurs).
...
Les inconvénients de cette approche, d'offrir des opérations de l'API à la gestion des assets, sont les suivants :
...
Les services de stockage d'objets sont généralement basés sur une architecture de stockage distribué. Les données sont répliquées sur plusieurs serveurs pour garantir la disponibilité et la durabilité des données.
...
Une URL SAS est une URL qui inclut une SAS, permettant aux utilisateur ou une application d'accéder à une ressource spécifique dans Azure Storage sans avoir besoin des clés d'accès principales du compte de stockage.
...
Comme vous le savez déjà, Azure Storage est un service cloud de stockage d'objets qui permet de stocker des fichiers binaires (ex: images, vidéos, fichiers audio...) de manière sécurisée et scalable. 
...
Dans un premier temps, il faut s'assurer qu'on aie le droit d'accéder de manière anonyme aux blobs. 
...
- **`@RequestPart("image")`** : permet de récupérer le fichier image envoyé dans le corps de la requête. Le type MIME de la requête doit être **`multipart/form-data`**.
...
Voici les modifications à faire dans le service de gestion des pizzas (dans le fichier **`services/PizzaService.java`**) pour mettre à jour la méthode **`createPizza`** :
...
- **`@Value("${azure.blob.container-name}") String containerName`** : injecte la valeur de la propriété **`azure.blob.container-name`** dans le champ **`containerName`**. Cette propriété provient du fichier **`application.properties`**, le fichier de configuration de l'API en mode développement.
...
Pour l'affichage, c'est le frontend, via une balise **`<img>`**, qui va chercher l'image stockée dans le service de stockage d'objets Azure Blob Storage.
...
Est-ce que c'est nécessaire de mettre dans le fichier de configuration de docker compose qui sera exécuté sur Azure Web app ? Non, ce qui compte, c'est de configurer les variables d'environnement dans le portail Azure.  
...
- Ajouter la variable d'environnement suivante en cliquant sur **`Add`** :
  - **`AZURE_BLOB_SAS_TOKEN`** (**`Name`**): **`sv=2022-11-02&ss=bfqt&srt=o&sp=rwdlacupiytfx&se=2035-02-24T23:52:15Z&st=2025-02-24T15:52:15Z&spr=https&sig=OoOAYhdQZGxO74hSUHKHgDw1Bvq%2FeSciIift10CMM34%3D`** (**`Value`**). Cliquez sur **`Apply`**.
