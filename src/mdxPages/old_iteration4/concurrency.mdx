...
Si une exception est levée dans une méthode annotée avec **`@Transactional`**, la transaction est automatiquement annulée et les opérations effectuées dans cette transaction sont annulées. Cela correspond à un **`rollback`** au niveau de la base de données.
Cela garantit que les données restent cohérentes. Si tout se passe bien, la transaction est validée et les opérations sont confirmées. Cela correspond à un **`commit`** au niveau de la base de données. 

Les transactions ne règlent pas les problèmes de concurrence que nous verrons plus tard. Néanmoins, si les transactions sont couplées à la gestion de la concurrence (optimistic ou pessimistic locking traités après la gestion des transactions), elles permettent de gérer les conflits de manière plus robuste.
...
💭 Est-ce que les transactions sont suffisantes dans cet exemple pour se protéger contre les accès concurrents ?  
Si deux transactions distinctes tentent de sauvegarder la même adresse ou le même utilisateur en même temps, il pourrait y avoir des conflits ou des incohérences.  
Cependant, l'optimistic ou le pessimistic locking (que nous verrons plus tard) n'est pas nécessaire ici car :
- La méthode **`save`** de JPA garantit que l'adresse ne sera pas dupliquée si une contrainte d'unicité est définie sur la colonne street (comme dans votre entité **`Address`** avec **`@Column(unique = true)`**).
- Si deux transactions tentent de sauvegarder la même adresse en même temps, la base de données lèvera une exception (par exemple, une **`ConstraintViolationException`**). La deuxième transaction sera annulée, garantissant que l'adresse n'est pas dupliquée.
...
Veuillez mettre à jour votre projet GitLab contenant la gestion des Pizzas pour ajouter la gestion des transactions.

Il suffit de rajouter l'annotation **`@Transactional`** sur les méthodes de service qui effectuent des opérations de mise à jour sur la base de données et qui dépendent du succès d'autres opérations.

Par exemple, au niveau de l'API de gestion de pizzas, lorsque nous ajoutons, ou effaçons des pizzas, nous faisons des opérations au niveau d'Azure Storage pour stocker ou effacer les images des pizzas.  
Si ces opérations échouent, nous ne voulons pas que les opérations de mise à jour de la base de données soient exécutées.  
Veuillez donc mettre à jour le service de gestion des pizzas nommé **`services/PizzaService`**.
...
Grâce à ces ajouts minuscules, si par exemple l'opération de sauvegarde de l'image échoue, l'opération de sauvegarde de la pizza ne sera pas exécutée. En effet, si la fonction **`uploadImageToBlob`** lève une exception, la transaction sera annulée et la pizza ne sera pas sauvegardée.  
Nous n'avons pas réellement besoin de transaction pour cette opération, car la sauvegarde de la pizza ne se fait que si l'opération de sauvegarde de l'image réussit. Néanmoins, nous pouvons facilement imaginer un scénario où la transaction serait nécessaire, par exemple :
- si nous devions sauvegarder la pizza avant de sauvegarder l'image.
- si nous devions d'abord sauvegarder les ingrédients de la pizza avant de sauvegarder la pizza, puis d'uploader l'image.

Dans ces cas, les transactions garantissent que toutes les opérations sur la DB sont exécutées ou aucune opération n'est exécutée.
...
public class Product {
    ...
    @Version
    private long version; // Champ utilisé pour l'optimistic locking
...
Veuillez mettre à jour votre projet GitLab contenant la gestion des Pizzas pour ajouter l'optimistic locking.

💭 Quelle serait l'opération la plus évidente qui pourrait être sujet à des problèmes de concurrence ?  
L'opération la plus évidente qui pourrait être sujette à des problèmes de concurrence est l'opération de mise à jour d'une pizza. En effet, si deux utilisateurs essaient de modifier la même pizza en même temps, il peut y avoir des problèmes de cohérence des données. Par exemple, si un utilisateur essaie de modifier le titre d'une pizza, et qu'un autre utilisateur essaie de modifier la description de la même pizza en même temps, il peut y avoir des problèmes de cohérence des données.

Pour ajouter l'optimistic locking à notre application, nous devons ajouter un champ de version à l'entité **`Pizza`** et utiliser cet attribut pour gérer les conflits de version lors de la mise à jour d'une pizza.

Veuillez donc mettre à jour l'entité **`Pizza`** pour ajouter un champ de version :
...
A l'aide du client HTTP d'IntelliJ, veuillez ajouter une pizza en exécutant la requête de création de pizza contenant une image contenue dans le fichier **`/tests/java/pizzas.http`**.

Puis, via l'onglet **`Database`** d'IntelliJ, veuillez vous connecter à votre DB local et vérifier que la pizza a bien été ajoutée.  
Vous devriez observer que la table **`Pizza`** de votre DB contient une nouvelle entrée avec un champ **`version`** initialisé à **`0`**.

A chaque opération de mise à jour d'une pizza, le champ **`version`** sera automatiquement incrémenté par JPA.

Veuillez exécuter la requête de mise à jour d'une pizza contenue dans le fichier **`/tests/java/pizzas.http`**.  
Vous devriez observer que la version de la pizza a été incrémentée dans la DB, passant de **`0`** à **`1`**.

Il est difficile de simuler un conflit de version avec le client HTTP d'IntelliJ, car les requêtes sont exécutées de manière séquentielle.  
Pour ce cours, nous ne simulerons pas de conflit de version, mais vous pouvez imaginer que si deux utilisateurs tentent de modifier la même pizza en même temps, il peut y avoir des problèmes de cohérence des données.  
Dans ce cas, voici le code déjà existant de **`PizzaService`** qui va soulever une exception **`OptimisticLockException`** si la version de la pizza en mémoire ne correspond pas à la version de la pizza en base de données :
...
En effet, lors du **`save`** de la pizza, JPA vérifie automatiquement si la version de la pizza en mémoire (retournée par **`pizzaRepository.findById(id).orElse(null)`**) correspond à la version de la pizza en base de données. Si ce n'est pas le cas, une exception **`OptimisticLockException`** est levée car ça signifierait qu'un autre utilisateur aurait mis à jour la pizza entre le moment du **`findById`** et du **`save`**.
...
### Principes pour gérer l'optimistic locking dans une API RESTful avec lecture suivie d'une mise à jour
...
public class UpdateProductRequest {
    ...
    private long version; // Version reçue lors de la lecture
...
Veuillez mettre à jour votre projet GitLab contenant la gestion des Pizzas et des Boissons pour ajouter le pessimistic locking.

Nous allons imaginer un scénario un peu tiré par les cheveux où 100 utilisateurs essaient en même temps de modifier une boisson. Nous sommes d'accord que c'est peu probable, mais cela nous permettra de simuler un cas de pessimistic locking.

Commençons par mettre à jour le repository **`DrinkRepository`** pour ajouter le pessimistic locking :
...
Nous avons ajouté l'annotation **`@Lock(LockModeType.PESSIMISTIC_WRITE)`** sur la méthode **`findById`** pour appliquer un verrou pessimiste en écriture sur la ressource. Cela signifie que la ressource renvoyée par cette méthode est verrouillée pour empêcher tout autre processus de la modifier.

Ensuite, mettons à jour le service **`DrinkService`** pour l'opération de mise à jour d'une boisson :
...
Nous avons ajouté l'annotation **`@Transactional(timeout = 10)`** pour limiter la durée de la transaction à 10 secondes. Si la transaction ne se termine pas dans les 10 secondes, une exception **`TransactionTimedOutException`** est levée et le verrou est libéré.

Via le client HTTP d'IntelliJ, nous ne pouvons pas simuler un cas de pessimistic locking, car les requêtes sont exécutées de manière séquentielle.  
Pour ce cours, nous ne simulerons pas de cas de pessimistic locking, mais vous pouvez imaginer que si 100 utilisateurs tentent de modifier la même boisson en même temps, il peut y avoir des problèmes de cohérence des données.  
Ici le système de verrous assure que seul un utilisateur à la fois peut modifier une boisson. Les autres utilisateurs doivent attendre que le verrou soit libéré.  
Bien sûr, ce type de scénario est peu probable dans la vraie vie, et est acceptable si l'on accepte que les données de la ressource puissent être modifiées à tour de rôle, garantissant la cohérence des données pour chaque modification, mais pas entre les modifications. 

En effet, si un utilisateur modifie une boisson, puis un autre utilisateur modifie la même boisson, la deuxième modification écrasera la première, sans que cela ne pose de problème de cohérence des données. 

Mais qui détient la vérité au niveau des données ? Le dernier utilisateur ? Si la réponse à cette question est oui, alors le pessimistic locking est une bonne solution. Si la réponse est non, alors il faudra trouver une autre solution pour gérer la concurrence.  

💭 Quelle genre de solution ?  
Une solution pourrait être d'utiliser l'optimistic locking, qui permet de gérer les conflits de version entre les modifications de la ressource.  Il faudra alors renvoyer des codes HTTP d'erreurs aux utilisateurs qui tentent de modifier une ressource qui a été modifiée entre-temps, et leur demander de recharger la ressource pour obtenir la version la plus récente.

Si nécessaire, vous pouvez trouver le code associé à la gestion de transactions et de la concurrence ici : [concurrency](https://github.com/e-vinci/cae-theory-demos/tree/main/concurrency).
