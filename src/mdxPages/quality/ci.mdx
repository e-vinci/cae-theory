...
L'ajout du boilerplate est à faire par un seul membre de l'équipe d'un projet, et vous devriez ensuite en discuter en groupe pour que tout le monde soit bien au courant de ce qui a été fait. 
...
3. Regarder les modifications apportées par la MR via l'onglet `Commits` et l'onglet `Changes`.
4. Si vous avez des remarques : 
- vous pouvez les ajouter dans l'onglet `Changes`. Pour chaque fichier modifié, vous pouvez ajouter des commentaires sur des lignes spécifiques.
...
5. Un autre membre de l'équipe, le `Reviewer`, fait une review des modifications et approuve la *MR* seulement s'il n'a plus de remarques. Tant qu'il y a des remarques, le développeur (le créateur de la MR) doit corriger les erreurs et pousser de nouveaux commits.
...
Si nécessaire, vous pouvez trouver le code associé à l'intégration continue ici : [ci](https://github.com/e-vinci/cae-theory/tree/main/ci).