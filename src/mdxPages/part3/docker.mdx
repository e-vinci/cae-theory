...
<InternalPageMenu>
  ...
  <InternalPageMenuItem> Comment construire les images dans notre pipeline ? </InternalPageMenuItem>
</InternalPageMenu>
...
Voici à quoi nous souhaitons que ressemble notre environnement multi-conteneurs, qui reproduit fidèlement ce que serait un environnement de production :

<PlantUML src="/diagrams/conteneurisation.puml" alt="test-environment"  />

Voici quelques explications sur les différents éléments de cet environnement :

- **Docker Host** : Représente le serveur hôte où Docker est installé. Il s'agit ici de votre ordinateur.
- **Frontend** : Conteneur pour l'application frontend (React), servie par nginx sous forme de site statique.
- **API** : Conteneur pour l'API (Spring Boot).
- **Database** (DB) : Conteneur pour la base de données (PostgreSQL).
- **Reverse proxy** : Conteneur nginx qui sert de point d'entrée unique et redirige les requêtes vers le frontend ou l'API.
...
Nous allons dans un premier temps voir comment créer une image pour un environnement de production.  
...
Lorsque nous souhaitons tester ou distribuer notre application, nous avons besoin de créer une image Docker optimisée pour un environnement de production. Cette image est légère, ne contient pas d'outils de développement, et sert l'application de manière efficace.
...
Comme la base de données est utilisée par le service **`api`**, nous avons ajouté un service **`db`**. Comme le service **`db`** est au sein du même `docker-compose` que le service **`api`**, le service **`api`** pourra communiquer avec le service **`db`** via le nom du service **`db`**.
...
- **`depends_on`** : 
  ...
  - Attention, **`depends_on`** ne garantit pas que le service dépendant est prêt avant de démarrer le service dépendant. Pour attendre que le service dépendant soit prêt, il faut utiliser des outils qu'il est possible d'ajouter à l'image Docker, comme **`wait-for-it.sh`** ou **`dockerize`**. Nous ne verrons probablement pas cela dans ce cours.

Nous avons également ajouté un **`entrypoint`** pour le service **`api`** : 
- L'**`entrypoint`** est une commande qui est exécutée lorsque le conteneur est démarré. Ici, nous exécutons l'application Spring Boot avec le profil `prod`. Nous avons vu dans le tutoriel sur les profils Spring Boot que nous pouvons utiliser différents profils pour adapter la configuration selon l'environnement.
...
Nous utilisons le profil **`prod`** pour que l'API utilise la configuration adaptée à l'environnement Docker. Nous devons donc nous assurer que le fichier **`application-prod.properties`** dans le répertoire **`src/main/resources`** de l'API contient la bonne configuration pour la base de données.

Veuillez mettre à jour (ou créer) le fichier **`application-prod.properties`** dans le répertoire **`src/main/resources`** de l'API :
...
Néanmoins, même si dans le terminal vous voyez que l'API est démarrée, vous ne pourrez pas accéder à l'API via l'adresse **`http://localhost:3000`** depuis le frontend. En effet, l'API est démarrée dans un conteneur Docker, et le frontend ne peut pas communiquer avec l'API. Nous allons voir comment configurer les conteneurs pour qu'ils puissent communiquer entre eux.
...
## Compréhension de l'environnement multi-conteneurs actuel

Dans l'environnement multi-conteneurs (proche de l'environnement de production final), toutes les applications sont dockerisées. Le frontend, l'API Spring Boot et la base de données PostgreSQL sont exécutés dans des conteneurs Docker.

Voici un schéma de l'environnement multi-conteneurs actuel :

<PlantUML src="/diagrams/local-test-deployment1.puml" alt="Environnement multi-conteneurs"/>
...
- Option 1 : faire en sorte que le frontend exécute directement les requêtes vers l'URL de l'API dans le conteneur Docker, c'est à dire **`http://api:3000`**. 
...
Nous allons opter pour l'**Option 3** qui est la plus élégante et la plus utilisée en production.
...
Voici un schéma de l'environnement multi-conteneurs avec un reverse proxy :

<PlantUML src="/diagrams/local-test-deployment2.puml" alt="Environnement multi-conteneurs avec un reverse proxy"/>
...
- **Point d'entrée unique** : le client n'a besoin de connaître qu'une seule URL (celle du reverse proxy), peu importe l'architecture interne de l'application.
...
- **`location /api/`** : 
  ...
  - **`proxy_pass http://api:3000/;`** : Redirige les requêtes vers le service **`api`**. Le nom **`api`** correspond au nom du service défini dans le **`docker-compose.yml`**. Docker Compose crée automatiquement un réseau interne qui permet aux services de se trouver par leur nom.
- **`location /`** :
...
Il ne reste plus qu'à ajouter le service **`reverse-proxy`** au **`docker-compose.yml`**. Veuillez mettre à jour le fichier **`docker-compose.yml`** à la racine de votre projet avec le contenu final suivant :
...
L'application est maintenant entièrement dockerisée. Vous pouvez ouvrir un navigateur et accéder à l'adresse **`http://localhost`** pour voir l'application frontend en production locale. Vous devriez voir l'application frontend servie par le reverse proxy **`nginx`**, et tout devrait être fonctionnel 🚀 !

Vous devriez aussi pouvoir accéder à l'API via l'adresse **`http://localhost/api`**.

## Récapitulatif de l'architecture multi-conteneurs

Voici un récapitulatif des éléments de l'architecture multi-conteneurs mise en place :

| Composant | Image Docker | Port interne | Port exposé sur l'hôte | Rôle |
|---|---|---|---|---|
| **frontend** | `nginx:alpine` (multi-stage build depuis `node:20-alpine`) | 5172 | 5172 | Sert les fichiers statiques du frontend |
| **api** | `eclipse-temurin:21-jre-jammy` (multi-stage build depuis `maven`) | 3000 | 3000 | Exécute l'API Spring Boot |
| **db** | `postgres:latest` | 5432 | 5432 | Base de données PostgreSQL |
| **reverse-proxy** | `nginx:alpine` | 80 | 80 | Point d'entrée unique, redirige vers frontend ou API |

Le **reverse-proxy** est le seul point d'entrée accessible depuis l'extérieur sur le port **`80`**. Il redirige :
- les requêtes **`/api/*`** → vers le service **`api`** sur le port **`3000`**
- toutes les autres requêtes → vers le service **`frontend`** sur le port **`5172`**

# <InternalPageTitle> Comment ignorer les fichiers qui ne peuvent pas se retrouver dans l'image ? </InternalPageTitle>

Nous avons déjà vu comment optimiser les images Docker en utilisant un fichier **`.dockerignore`** pour exclure les fichiers inutiles. Rappelons l'utilité de ce fichier.
...
Il est important de penser à ajouter dans le fichier **`.dockerignore`** tous les fichiers et répertoires que vous ne souhaitez pas inclure dans l'image Docker, comme :
- le répertoire **`node_modules`** pour éviter de copier les dépendances de l'hôte dans l'image (elles seront réinstallées dans le conteneur).
- le répertoire **`dist`** pour éviter de copier les fichiers de build de l'hôte dans l'image (ils seront régénérés dans le conteneur).
- l'éventuel **`.git`** et **`.gitignore`**.
- l'éventuel **`.env`** pour ne pas exposer les secrets dans l'image !
- le **`Dockerfile`** lui-même, les fichiers **`README`**, etc.

Voici tout ce que nous pourrions exclure de l'image Docker du frontend :
...
Et pour l'API (dans **`/api/.dockerignore`**) :
...
Pour l'API, nous excluons le répertoire **`target`** car il contient les fichiers compilés de l'application. Ces fichiers sont inutiles dans l'image Docker car l'application est compilée lors de la construction de l'image à l'aide de Maven.

# <InternalPageTitle> Comment construire les images dans notre pipeline ? </InternalPageTitle>

## Introduction

Maintenant que nous avons dockerisé notre application, nous pouvons automatiser la construction des images Docker dans notre pipeline GitLab. L'objectif est de s'assurer que les images Docker peuvent être construites correctement à chaque push sur le dépôt.

## Configuration du pipeline pour construire les images

Nous souhaitons exécuter ces stages en séquence dans notre pipeline :
1. **`test`** : Exécuter les tests unitaires et les analyses statiques de notre application.
2. **`build`** : Construire les images Docker de notre application.

Le stage **`build`** vérifie que les images Docker peuvent être construites sans erreur. C'est une validation importante, car une image qui ne se construit pas empêchera l'application de fonctionner.

Veuillez mettre à jour le fichier **`.gitlab-ci.yml`** à la racine de votre projet avec le contenu suivant :

```yaml {1-4,42-50} showLineNumbers
stages:
  - test
  - build
...
- Nous utilisons le service **`docker:20-dind`** pour exécuter Docker dans Docker (DinD). C'est nécessaire car notre runner GitLab ne dispose pas de Docker par défaut.
- Nous utilisons la commande **`docker compose build`** pour construire toutes les images Docker de notre application (frontend, api).

Ce stage s'assure que les images Docker peuvent être construites sans erreur. Si une image ne peut pas être construite, le pipeline échouera et les développeurs seront immédiatement alertés.

Il suffit de pousser votre branche sur votre projet GitLab pour déclencher le pipeline.

## Conclusion sur la conteneurisation

Nous avons mis en place un environnement multi-conteneurs complet qui reproduit fidèlement un environnement de production. Grâce à **Docker Compose**, il est possible de démarrer l'intégralité de l'application (frontend, API, base de données, reverse proxy) en une seule commande :

```bash
docker compose up --build
```

Dans notre pipeline GitLab, nous construisons automatiquement les images Docker à chaque push pour vérifier que tout peut être construit correctement.

Voici ce que nous avons accompli :
- Création d'un **`Dockerfile`** optimisé pour le frontend (multi-stage build avec nginx).
- Création d'un **`Dockerfile`** optimisé pour l'API (multi-stage build avec Maven et JRE).
- Configuration d'un **reverse proxy nginx** pour servir l'application depuis un point d'entrée unique.
- Configuration de **Docker Compose** pour orchestrer tous les services.
- Ajout d'un stage **`build`** dans notre pipeline GitLab pour valider la construction des images.

C'est une grande avancée pour nos projets 🚀 ! L'application est maintenant entièrement dockerisée et peut fonctionner de manière cohérente sur n'importe quelle machine disposant de Docker.
