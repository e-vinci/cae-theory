...
- **FATAL** : Erreurs graves qui ont provoqué l'arrêt de l'application. Il est à noter que la librairie **Logback** ne supporte pas ce niveau de log. **FATAL** est donc souvent remplacé par **ERROR**.
...
## Gérer des exceptions personnalisées
...
💭 Comment tester le passage dans la méthode globale **`handleGlobalException`** ?  Pour cela, vous pouvez par exemple, stopper votre base de données et tenter de lire une pizza.
...
Cela est impressionnant, nous sommes maintenant capable de "catcher" toutes les exceptions non traitées par les contrôleurs et de les logguer de manière centralisée.
...
Veuillez aussi observer que le message d'erreur renvoyé au client ne contient pas ces informations sensibles !
...
Dans notre cours, nous avons choisi de déployer l'API dans Azure Web App à l'aide de Docker Compose. De plus, pour les assets, nous utilisons Azure Blob Storage. Nous allons donc stocker les logs dans Azure Blob Storage pour les rendre persistants et les consulter facilement.
...
Nous vous avons jusque là recommandé de créer vos services Azure dans la région **`East US`**. Pourquoi ? C'est aussi la région la moins chère pour les étudiants Azure.
...
### Pour stocker les logs et les visualiser facilement via Azure Monitor, vous pouvez activer les diagnostics de l'API dans Azure Web App. Pour cela, veuillez suivre ces étapes :
- Connectez-vous au portail Azure.
- Recherchez et sélectionnez votre **`App Service`**.
...
## Activer les diagnostics dans l'application

### Activer les logs dans Azure Web App
...
### Activer l'accès FTPS

Nous souhaitons accéder aux logs de l'API via FTPS. Pour cela, veuillez suivre ces étapes :
- Connectez-vous au portail Azure.
- Recherchez et sélectionnez votre **`App Service`**.
- Dans le menu de gauche, dans **`Settings`**, sélectionnez **`
Configuration`**.
- Pour **`FTP Basic Auth Publishing Credentials`**, sélectionnez **`On`**.
- Cliquez sur **`Save`**.
- Dans le menu de gauche, dans **`Deployment`**, sélectionnez **`Deployment Center`**.
- Cliquez sur **`FTPS credentials`** pour accéder aux logs via FTPS.
- Notez les informations de connexion FTPS, **`FTPS endpoint`** (**`ftps://waws-prod-dm1-173.ftp.azurewebsites.windows.net/site/wwwroot`** par exemple)
- Créer un **`Username`** avec le bon format, par exemple **`caeuser007`** (doit être unique au sein de Azure !) et un **`Password`** respectant aussi le bon format, comme **`Metoo123!*`**. Pour se logguer, le username sera quelque chose du style **`staging-pizzeria\caeuser007`** (voir dans le portail, il s'agit de d'abor indiquer le nom de l'app) et le password **`Metoo123!*`**.

## Accéder aux logs

Comment accéder à ces logs ? Pour accéder à ces logs, vous pouvez soit utiliser les **`Advanced Tools`** de l'Azure Web App. Pour cela, veuillez suivre ces étapes :
- Connectez-vous au portail Azure.
- Recherchez et sélectionnez votre **`App Service`**.
...
- Cliquez sur **`Log stream`** pour visualiser les logs en temps réel.

Sinon, vous pouvez aussi accéder aux logs via FTP. Pour cela, veuillez suivre ces étapes :
- Récupérez les informations de connexion FTPS que vous avez notées précédemment.
- Utilisez un client FTP pour vous connecter à l'Azure Web App, comme par exemple votre explorateur de fichiers.
- Naviguez dans le dossier **`LogFiles`** pour accéder aux logs de l'API.

## Visualiser les logs dans Azure Blob Storage
...
Néanmoins, il serait plus facile d'avoir une application qui permet de visualiser les logs en temps réel. Pour cela, vous pourriez utiliser Azure Monitor pour visualiser les logs en temps réel.


## Utiliser Azure Monitor pour visualiser les logs en temps réel

### Introduction

Azure Monitor est un service Azure qui permet de visualiser les logs en temps réel. Pour l'utiliser, vous devez créer un **`Log Analytics workspace`** et connecter votre Azure Web App à ce workspace.
...
- En cliquant sur **`Simple mdoe`**, vous pouvez switcher vers le mode **`KQL mode`** pour écrire des requêtes plus avancées sur les logs de l'API.
...
Dans ce tutoriel, nous avons vu comment visualiser les logs dans l'environnement déployé. Nous avons vu comment stocker les logs dans Azure Blob Storage pour les rendre persistants et les consulter (facilement) pour suivre les erreurs de manière cohérente et les corriger. Nous avons aussi vu comment utiliser Azure Monitor pour visualiser les logs en temps réel.

Nous comprenons que la gestion des logs dépend fortement de chaque provider cloud. N'hésitez pas à consulter la documentation de votre provider cloud pour plus d'informations sur la gestion des logs.

Une autre façon intéressante de gérer la persistance des logs pourrait être d'utiliser Spring pour directement stocker les logs dans Azure Blob Storage. Cela pourrait être une piste intéressante pour un projet futur.
...
Nous utilisons déjà Azure Blob Storage pour stocker les images de l'API. Nous allons donc simplement créer un volume Docker pour stocker les logs dans Azure Blob Storage.

Il existe plusieurs options pour stocker les logs dans Azure Blob Storage :
- Soit directement mettre à jour l'image de l'API pour écrire les logs dans Azure Blob Storage.
- Soit créer un nouveau service dans Docker Compose pour stocker les logs dans Azure Blob Storage.
- Soit au niveau de la pipeline de GitLab CI/CD, stocker les logs dans Azure Blob Storage.

Pour ce cours, nous avons choisi la deuxième option qui consiste à simplement créer un nouveau service dans Docker Compose pour stocker les logs dans Azure Blob Storage.

## Créer un nouveau service dans Docker Compose pour stocker les logs dans Azure Blob Storage

Veuillez mettre à jour le fichier **`docker-compose.staging.yml`** pour ajouter un nouveau service **`blobfuse`** qui stocke les logs dans Azure Blob Storage en temps réel :

```yaml {2-14,32-33,48-49} showLineNumbers
services:
  blobfuse:
    image: mcr.microsoft.com/azure-blobfuse2:latest
    container_name: blobfuse
    privileged: true # Nécessaire pour utiliser FUSE
    environment:
      - AZURE_STORAGE_ACCOUNT=imagestorage007
      - AZURE_BLOB_CONTAINER=staging
      - AZURE_BLOB_SAS_TOKEN=${AZURE_BLOB_SAS_TOKEN}
    volumes:
      - blobfuse-logs:/mnt/blob # Monte le conteneur Azure Blob Storage dans /mnt/blob
    entrypoint: >
      blobfuse2 mount /mnt/blob
      --config-file=/blobfuse2-config.yaml
  ...
  api:
    ...
    ports:
      - 3000:3000
    volumes:
      - blobfuse-logs:/api/logs # Partage le volume monté par Blobfuse avec le conteneur API
    environment:
...
Veuillez aussi mettre à jour le fichier **`docker-compose.deploy.yml`** pour ajouter un nouveau service **`blobfuse`** qui stocke les logs dans Azure Blob Storage en temps réel :

```yaml {2-14,29-30,44-45} showLineNumbers
services:
  blobfuse:
    image: mcr.microsoft.com/azure-blobfuse2:latest
    container_name: blobfuse
    ...
    environment:
      - AZURE_STORAGE_ACCOUNT=imagestorage007
      - AZURE_BLOB_CONTAINER=production
      - AZURE_BLOB_SAS_TOKEN=${AZURE_BLOB_SAS_TOKEN}
    volumes:
      - blobfuse-logs:/mnt/blob # Monte le conteneur Azure Blob Storage dans /mnt/blob
    entrypoint: >
...
Il faut aussi ajouter la variable d'environnement **`AZURE_BLOB_SAS_TOKEN`** dans GitLab CI/CD pour stocker les logs dans Azure Blob Storage. Pour cela, veuillez ajouter la variable d'environnement **`AZURE_BLOB_SAS_TOKEN`** dans les variables d'environnement de votre pipeline de déploiement.  

Pour rappel :
- Dans votre projet GitLab, allez dans **`Settings`** > **`CI/CD`** > **`Variables`**.
- Cliquez sur **`Add variable`**.
- Déselectionnez **`Protected`** pour que la variable soit accessible dans les branches protégées.
- Ajoutez la variable **`AZURE_BLOB_SAS_TOKEN`** avec la valeur de votre **`SAS Token`**.



## Création d'un Azure File Share pour stocker les logs dans Azure Blob Storage

Pour stocker les logs dans Azure Blob Storage, nous devons créer un Azure File Share pour stocker les logs dans Azure Blob Storage. Pour cela, veuillez suivre les étapes suivantes :
- Connectez-vous à votre compte Azure sur le portail Azure : https://portal.azure.com/
- Sélectionnez votre Storage account, par exemple **`imagestorage007`**.
- Cliquez sur **`File shares`** dans le menu de gauche au sein de **`Data storage`**.
- Donnez un nom à votre **`File share`**, par exemple **`staging-cae-logs`**.
- Cliquez sur **`Review + Create`** pour créer le **`File share`**.
- Cliquez sur **`Create`** pour créer le **`File share`**.

## Monter le Azure File Share dans la Web App

Pour monter le **`Azure File Share`** dans la **`Web App`**, veuillez suivre les étapes suivantes :
- Connectez-vous à votre compte Azure sur le portail Azure : https://portal.azure.com/
- Sélectionnez votre **`Web App`**, par exemple **`staging-cae-projects`**.
- Cliquez sur **`Configuration`** dans le menu de gauche.
- Cliquez sur **`Path mappings`**.
- Cliquez sur **`New Azure file share`**.
- Ecrivez votre **`Storage account`**, par exemple **`imagestorage007`**.
- Donnez un nom à votre **`Azure File Share`**, par exemple **`logs`**.
- Pour **`Storage type`**, sélectionnez **`Azure Files`**.
- Sélectionnez votre **`Share name`**, par exemple **`staging-cae-logs`**.
- Comme **`Access key`**, sélectionnez et copier **`Key1`** à partir de votre **`Storage account`**, dans **`Security + networking`** > **`Access keys`**.
- Pour le **`Mount path`**, sélectionnez **`/mnt/logs`**.
- Cliquez sur **`OK`** pour monter le **`Azure File Share`** dans la **`Web App`**.

## Monter le File Share au sein de l'API

Veuillez mettre à jour le fichier **`application-staging.properties`** pour stocker les logs dans le **`Azure File Share`** :

```properties
logging.file.name=/mnt/logs/application.log
```

Veuillez mettre à jour le fichier **`application-production.properties`** pour stocker les logs dans le **`Azure File Share`** :
